...
class QLearner(object):
    ...
    def __init__(
        ...
        target_val = self.rew_t_ph + gamma * (1.0 - self.done_mask_ph) * max_q
        q_val = tf.reduce_sum(self.q * tf.one_hot(self.act_t_ph, self.num_actions), axis = 1)

        #self.total_error = tf.losses.mean_squared_error(target_val, q_t_taken)
        self.total_error = huber_loss(x=target_val - q_val)
    ...
    def update_model(self):
        ...
        if (self.t > self.learning_starts and \
            ...
            # YOUR CODE HERE
            obs_batch, act_batch, rew_batch, obs_next_batch, done_batch = self.replay_buffer.sample(self.batch_size)
            if not self.model_initialized:
                initialize_interdependent_variables(self.session, tf.global_variables(), {
                        self.obs_t_ph: obs_batch,
                        self.obs_tp1_ph: obs_next_batch
                    })
                self.model_initialized = True

            _, loss = self.session.run([self.train_fn, self.total_error],
                feed_dict={
                    self.obs_t_ph: obs_batch,
    ...
    def log_progress(self):
        ...
        if self.t % self.log_every_n_steps == 0:
            ...
            logz.log_tabular("mean reward (100 episodes)", self.mean_episode_reward)
