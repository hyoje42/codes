...
seeds, call
    ...
    python plot.py data/test_EnvName_DateTime --value "mean reward (100 episodes)"
...
def plot_data(data, values=['mean reward (100 episodes)', 'best mean reward']):
    if isinstance(data, list):
        data = pd.concat(data, ignore_index=True)

    data = data.melt(id_vars='timestep', value_vars=values, value_name='values', var_name='columns')

    sns.set(style="darkgrid", font_scale=1.0)
    sns.lineplot(data=data, x="timestep", y='values', hue='columns')
...
def main():
    ...
    parser.add_argument('--value', default=['mean reward (100 episodes)', 'best mean reward'], nargs='*')
